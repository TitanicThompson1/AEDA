...
void REAgency::generateCatalog() {
    ...
    for (auto it=properties.begin();it!=properties.end();it++){
        for(auto it2=temp.begin();it2!=temp.end();it2++){
            if(it2->getTypology() == (*it)->getTypology() && it2->getPostalCode() == (*it)->getPostalCode()){
                it2->addItems(*it);
                if(it2->getMaxPrice() < (*it)->getPrice()){
                    it2->setMaxPrice((*it)->getPrice());
                }
                found=true;
                break;
            }
        ...
        if(!found){
            PropertyTypeItem tempP((*it)->getAddress(),(*it)->getPostalCode(),(*it)->getTypology(),(*it)->getPrice());
            tempP.addItems(*it);
...
vector<Property*> REAgency::getAvailableProperties(Property* property) const {
    ...
    tuple<Client *,int> tempT;
	for(auto it=properties.begin();it!=properties.end();it++){
        if((*it)->getReservation()==tempT && (*(*it)) == *property){
